...
func (p *playlistServer) HandlerFunc() http.HandlerFunc {
	...
	router.HandleFunc("/live/{channel}.m3u8", p.CORSHandlerFunc(p.HandleMasterPlaylist)).Methods("GET")
	router.HandleFunc("/live/{channel}.m3u8", p.CORSHandlerFunc(p.HandleOptions)).Methods("OPTIONS")
	router.HandleFunc("/live/{channel}/index.m3u8", p.CORSHandlerFunc(p.HandleMediaPlaylist)).Methods("GET")
	router.HandleFunc("/live/{channel}/index.m3u8", p.CORSHandlerFunc(p.HandleOptions)).Methods("OPTIONS")
